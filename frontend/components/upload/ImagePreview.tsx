...
export function ImagePreview({ imageUri, onReplace, onRemove, onZoom }: ImagePreviewProps) {
  ...
  const zoomStyle = useAnimatedStyle(() => ({ transform: [{ scale: scaleZoom.value }] }));
  const replaceStyle = useAnimatedStyle(() => ({ transform: [{ scale: scaleReplace.value }] }));
  const removeStyle = useAnimatedStyle(() => ({ transform: [{ scale: scaleRemove.value }] }));
  ...
  return (
    ...
    >
      <View className="w-full h-72 rounded-xl bg-slate-900 border border-border/25 dark:border-border-dark/30 overflow-hidden relative justify-center items-center">
        ...
        <View className="absolute inset-0 border border-dashed border-white/5" style={{ pointerEvents: 'none' }} />
        <View className="absolute top-1/2 left-0 right-0 h-[0.5px] bg-white/20" style={{ pointerEvents: 'none' }} />
        <View className="absolute left-1/2 top-0 bottom-0 w-[0.5px] bg-white/20" style={{ pointerEvents: 'none' }} />
