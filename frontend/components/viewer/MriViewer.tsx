...
export function MriViewer({
  ...
  const showOverlay = viewMode === 'overlay';
  const activeSource = showOverlay && overlayUri
    ? { uri: overlayUri }
    : originalUri
      ? { uri: originalUri }
      : null;
      ...
      <GestureDetector gesture={allGestures}>
        <Animated.View className="w-full h-80 items-center justify-center relative bg-slate-950 overflow-hidden">
          <Animated.View style={animatedStyle} className="w-full h-full">
            {activeSource ? (
              <Image source={activeSource} className="w-full h-full" resizeMode="contain" />
            ) : (
            ...
            {showOverlay && overlayUri && originalUri && overlayOpacity > 0 && (
          ...
          <View className="absolute bottom-3 left-3 bg-black/60 px-2 py-0.5 rounded">
            <Text className="text-[8px] font-bold text-white uppercase font-mono">
              {showOverlay ? 'U-Net Composite Mask' : 'Raw Structural Slice'}
            </Text>
          </View>
        </Animated.View>
      </GestureDetector>
